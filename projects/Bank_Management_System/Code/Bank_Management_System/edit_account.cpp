...
void Edit_Account::on_comboBox_2_currentTextChanged(const QString &arg1)
{
    ...
    if (arg1 == "Loan Account") {
        ...
        qDebug() << "this is Checking account";
...
int Edit_Account::Check_Authorization()
{
    ...
    {
        qDebug() <<"Passowrd is empty" ;
        ui->lineEdit_Password->setText("*");
        flag =true;
    }
    if(Customer__NN.isEmpty())
    {
        qDebug() <<"National Number is empty" ;
    ...
    if(!flag)
    {
        ...
        {
            qDebug() <<"National Number not found" ;
...
void Edit_Account::on_pushButton_clicked()
{
    ...
    if(Customer_ID > 0)
    {
        ...
        case Account_Type::LOAN_ACCOUNT:
        {
            ...
            if(Loan_Amount == "No")
            {
                qDebug() <<"You haven't Loan account" ;
        ...
        case Account_Type::SAVING_ACCOUNT:
        {
            ...
            if(Saving_Amount == "No")
            {
                qDebug() <<"You haven't Saving account" ;
    ...
    if(Customer_ID > 0)
    {
        ...
        case Account_Type::LOAN_ACCOUNT:
        {
            ...
            else
            {
                ...
                QString sql;
                qDebug()<<Loan_Duration;
                qDebug()<<Loan_Duration.toInt();
                ...
                {
                    // QMessageBox_Display("The new interest rate is 0.5",QMessageBox::Information);
                    qDebug()<<"The new interest rate is 0.5";
                    Interest_Rate = 0.5;

                }
                else if((Loan_Duration.toInt() > 6) &&(Loan_Duration.toInt() <=12))
                {
                    // QMessageBox_Display("The new interest rate is 1.0",QMessageBox::Information);
                    qDebug()<<"The new interest rate is 1.0";

                    Interest_Rate = 1.0;
                }
                else if((Loan_Duration.toInt() > 12) && (Loan_Duration.toInt() <=18))
                {
                    // QMessageBox_Display("The new interest rate is 1.5",QMessageBox::Information);
                    qDebug()<<"The new interest rate is 1.5";

                    Interest_Rate = 1.5;
                }
                else if((Loan_Duration.toInt() > 18) && (Loan_Duration.toInt() <=24))
                {
                    // QMessageBox_Display("The new interest rate is 2.0",QMessageBox::Information);
                    qDebug()<<"The new interest rate is 2.0";
        ...
        case Account_Type::CHECKING_ACCOUNT:
        {
            ...
            else
            {
                float Credit_Limit = Return_Item("Account","Credit_Limit",QString("WHERE Customer_ID = %1 AND Account_ID=%2")
                                                                               .arg(Customer_ID)
                                                                               .arg(3)).toFloat();
                qDebug () <<"Credit_Limit is "<<Credit_Limit;
