...
bool BankDataBase::checkTableExists(QString tableName)
{
    ...
    qDebug() << "Checking if table exists:" << sql;
...
void BankDataBase::Daily_Updating(void)
{
    ...
    while (query.next()) {
        ...
        qDebug() << "Processing record #" << count
        ...
        if (operationDate.addMonths(1) <= currentDate) {
            ...
            if (accountId == 1) {
                if (loanDuration - 1 == 0) {
                    qDebug() << "Loan duration has ended. Inform admin.";
                }
                newBalance = balance - (Principle_Amount * interestRate / 100);
                qDebug() << "This is a loan account, new balance:" << newBalance;
            ...
            // Saving Account
                ...
                newBalance = balance + (Principle_Amount * interestRate / 100);
                qDebug() << "This is a savings account, new balance:" << newBalance;
            ...
            // Checking Account
                ...
                newBalance = balance - (Principle_Amount * interestRate / 100);
                qDebug() << "This is a checking account, new balance:" << newBalance;
            ...
            if (!updateQuery.exec()) {
                qDebug() << "Failed to update account:" << updateQuery.lastError().text();
            } else {
                qDebug() << "Account updated successfully for Customer ID:" << customerId;
            }
        }
    }

    qDebug() << "Total records processed: " << count;
...
void BankDataBase::Write_Summary()
{
    ...
    if (!query->exec(sql_person_count)) {
        qDebug() << "Error fetching Number_Of_Members: " << query->lastError().text();
        return;
    }
    ...
    if (!query->exec()) {
        qDebug() << "Error fetching Deposit_Amount: " << query->lastError().text();
        return;
    }
    qDebug() <<"sql_deposit_amount is "<<sql_deposit_amount;

    double deposit_amount = 0;
    if (query->next()) {
        deposit_amount = query->value(0).toDouble();
    }
    qDebug() <<"deposit_amount is "<<deposit_amount;
    ...
    qDebug() <<"sql_withdraw_amount is "<<sql_withdraw_amount;

    if (!query->exec()) {
        qDebug() << "Error fetching Withdraw_Amount: " << query->lastError().text();
        return;
    }
    ...
    qDebug() <<"withdraw_amount is "<<withdraw_amount;
    ...
    if (!query->exec()) {
        qDebug() << "Error inserting summary: " << query->lastError().text();
        return;
    }

    qDebug() << "Daily summary written successfully!";
}
